...
type
  ...
  TError = (e_Abort, e_Heap, e_OutOfMem, e_InOut, e_External, e_Int, e_DivBy0, e_Range, e_IntOverflow, e_Math, e_Math_InvalidArgument, e_InvalidOp, e_ZeroDivide, e_Overflow, e_Underflow, e_InvalidPointer, e_InvalidCast, e_Convert, e_AccessViolation, e_Privilege, e_StackOverflow, e_CtrlC, e_Variant, e_PropReadOnly, e_PropWriteOnly, e_Assertion, e_Abstract, e_IntfCast, e_InvalidContainer,
    e_InvalidInsert, e_Package, e_Win32, e_SafeCall, e_License, e_Custom, e_Com, e_Ole, e_Registry);
  ...
  public
    ...
    constructor CreateErr(ACode: TError; const Msg: string);
...
function EnumStringModules(HInstance: NativeInt; Data: Pointer): Boolean;
begin
  with PStrData(Data)^ do
  begin
    nChars := LoadString(HInstance, Ident, Buffer, BufSize);
    Result := nChars = 0;
  end;
end;
...
begin
  ...
  if (Info.State <> MEM_COMMIT) or (GetModuleFilename(THandle(Info.AllocationBase), { Temp } ModuleName, SizeOf( { Temp } ModuleName)) = 0) then
  begin
...
{$IFDEF UNICODE_CTRLS} WStrCopy {$ELSE} StrCopy {$ENDIF}
    (Buffer, PKOLChar(Format(Fmt, [ExceptObject.ClassName, ModuleName, ConvertedAddress, MsgPtr, '' { MsgEnd } ])));
...
procedure Abort;
  ...
  begin
    raise Exception.CreateErr(e_Abort, SOperationAborted)at ReturnAddr;
  end;
...
constructor Exception.CreateResFmt(ACode: TError; Ident: Integer; const Args: array of const);
begin
...
constructor Exception.CreateErr(ACode: TError; const Msg: string);
begin
  FCode := ACode;
  FMessage := Msg;
  // FAllowFree := TRUE;
...
constructor Exception.CreateCustomFmt(AError: DWORD; const Msg: String; const Args: array of const);
begin
  FCode := e_Custom;
  FErrorCode := AError;
...
constructor Exception.CreateFmt(ACode: TError; const Msg: string; const Args: array of const);
begin
  FCode := ACode;
  FMessage := Format(Msg, Args);
end;
...
const
  ErrorMap: array [0 .. 5] of TErrorRec = ((Code: 2; Ident: SFileNotFound), (Code: 3; Ident: SInvalidFilename), (Code: 4; Ident: STooManyOpenFiles), (Code: 5; Ident: SAccessDenied), (Code: 100; Ident: SEndOfFile), (Code: 101; Ident: SDiskFull) { ,
...
begin
  ...
  if I <= High(ErrorMap) then
    Result := Exception.CreateErr(e_InOut, ErrorMap[I].Ident)
  else
...
const
  ExceptMap: array [1 .. 24] of TExceptMapRec = ((ECode: e_OutOfMem; EIdent: SOutOfMemory), (ECode: e_InvalidPointer; EIdent: SInvalidPointer), (ECode: e_DivBy0; EIdent: SDivByZero), (ECode: e_Range; EIdent: SRangeError), (ECode: e_IntOverflow; EIdent: SIntOverflow), (ECode: e_InvalidOp; EIdent: SInvalidOp), (ECode: e_ZeroDivide; EIdent: SDivByZero), (ECode: e_Overflow;
    EIdent: SOverflow), (ECode: e_Underflow; EIdent: SUnderflow), (ECode: e_InvalidCast; EIdent: SInvalidCast), (ECode: e_AccessViolation; EIdent: SAccessViolation), (ECode: e_Privilege; EIdent: SPrivilege), (ECode: e_CtrlC; EIdent: SControlC),
    // {-} Only for console applications
    (ECode: e_StackOverflow; EIdent: SStackOverflow),
{$IFDEF VARIANT_USED}
    (ECode: e_Variant; EIdent: SInvalidVarCast), (ECode: e_Variant; EIdent: SInvalidVarOp), (ECode: e_Variant; EIdent: SDispatchError), (ECode: e_Variant; EIdent: SVarArrayCreate), (ECode: e_Variant; EIdent: SVarNotArray), (ECode: e_Variant; EIdent: SVarArrayBounds),
{$ELSE}
    (ECode: e_Variant; EIdent: SVar), (ECode: e_Variant; EIdent: SVar), (ECode: e_Variant; EIdent: SVar), (ECode: e_Variant; EIdent: SVar), (ECode: e_Variant; EIdent: SVar), (ECode: e_Variant; EIdent: SVar),
{$ENDIF}
    (ECode: e_Assertion; EIdent: SAssertionFailed), (ECode: e_External; EIdent: SExternalException), (ECode: e_IntfCast; EIdent: SIntfCastError), (ECode: e_SafeCall; EIdent: SSafecallException));
...
begin
  ...
  if ErrorCode <= 24 then
    with ExceptMap[ErrorCode] do
      E := Exception.CreateErr(ECode, EIdent)
  else
    E := CreateInOutError;
  { - }
...
function CreateAssertException(const Message, Filename: string; LineNumber: Integer): Exception;
var
  S: string;
begin
  if Message <> '' then
    ...
    S := SAssertionFailed;
  Result := Exception.CreateFmt(e_Assertion, SAssertError, [S, Filename, LineNumber]);
end;
...
begin
  raise Exception.CreateErr(e_Abstract, SAbstractError);
end;
...
begin
  case P.ExceptionCode of
    ...
    STATUS_INTEGER_OVERFLOW:
      Result := 5;
    STATUS_FLOAT_INEXACT_RESULT, STATUS_FLOAT_INVALID_OPERATION, STATUS_FLOAT_STACK_CHECK:
      Result := 6;
    STATUS_FLOAT_DIVIDE_BY_ZERO:
      Result := 7;
    STATUS_FLOAT_OVERFLOW:
      Result := 8;
    STATUS_FLOAT_UNDERFLOW, STATUS_FLOAT_DENORMAL_OPERAND:
      Result := 9;
...
var
  ...
  begin
    ...
    begin
      ...
      if (MemInfo.State = MEM_COMMIT) and (GetModuleFilename(THandle(MemInfo.AllocationBase), ModName, SizeOf(ModName)) <> 0) then
        Result := Exception.CreateFmt(e_AccessViolation, SModuleAccessViolation, [ExceptionAddress, ExtractFileName(ModName), AccessOp, AccessAddress])
      else
        Result := Exception.CreateFmt(e_AccessViolation, SAccessViolation, [ExceptionAddress, AccessOp, AccessAddress]);
    end;
  end;
...
begin
  ...
  case ErrorCode of
    3 .. 10, 12 .. 21:
      with ExceptMap[ErrorCode] do
        Result := Exception.CreateErr(ECode, EIdent);
    11:
...
begin
  ...
  if LastError <> ERROR_SUCCESS then
    Error := Exception.CreateFmt(e_Win32, SWin32Error, [LastError, SysErrorMessage(LastError)])
  else
    Error := Exception.CreateErr(e_Win32, SUnkWin32Error);
...
procedure ALV;
begin
  raise Exception.CreateErr(e_License, SNL);
end;
...
begin
  if MainInstance <> 0 then
    Result := Pointer(LoadResource(MainInstance, FindResource(MainInstance, 'DVCLAL', PKOLChar(RT_RCDATA))))
  else
  begin
    Result := nil;
    ...
    begin
      with LibModule^ do
      begin
        Result := Pointer(LoadResource(Instance, FindResource(Instance, 'DVCLAL', PKOLChar(RT_RCDATA))));
        if Result <> nil then
          Break;
      end;
...
begin
  ...
  PAL1s := @AL1s;
  PAL2s := @AL2s;
  ALOK := ((A1 = PAL1s[0]) and (A2 = PAL2s[0])) or ((A1 = PAL1s[1]) and (A2 = PAL2s[1])) or ((A1 = PAL1s[2]) and (A2 = PAL2s[2]));
